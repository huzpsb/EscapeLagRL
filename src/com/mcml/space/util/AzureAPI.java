...
public abstract class AzureAPI {
   ...
   public static int viewDistanceBlock(Player player) {
      return customViewDistance(player) ? player.getViewDistance() * 16 : viewDistanceBlock;
   }

   public static int viewDistanceChunk(Player player) {
      return customViewDistance(player) ? player.getViewDistance() * 2 ^ 3 : squaredViewDistanceChunk;
   }
   ...
   public static String concatsBetween(List<String> list, int start, int end, String spilt) {
      ...
      for(concated = ""; start <= end; ++start) {
         concated = concated.concat(((String)list.get(start)).concat(start == end ? "" : spilt));
      }
   ...
   public static <E> E colorzine(E element, Class<E> type) {
      if (element instanceof String) {
         return StringUtils.replaceChars((String)element, '&', '§');
   ...
   public static void dumpThread(ThreadInfo thread, Logger logger) {
      if (VersionLevel.isSpigot()) {
         try {
            ...
            dumpthreadmd.invoke((Object)null, thread, logger);
            ...
            Logger.getLogger(AzureAPI.class.getName()).log(Level.SEVERE, (String)null, var4);
         }
      }
   ...
   public static class Coord<K, V> {
      ...
      public boolean equals(Object o) {
         if (o == this) {
            ...
            AzureAPI.Coord<?, ?> other = (AzureAPI.Coord)o;
            if (!other.canEqual(this)) {
               return false;
            } else {
               Object this$key = this.getKey();
   ...
   public static class ChunkCoord {
      ...
      public int hashCode() {
         int PRIME = true;
         int result = 1;
         int result = result * 59 + this.getChunkX();

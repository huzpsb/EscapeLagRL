        ...
        if (Optimizes.TeleportPreLoaderenable) {
            ...
            if (!this.isPreLoading) {
                ...
                Bukkit.getScheduler().runTaskLater(EscapeLag.plugin, () -> {
                    this.nowint.put(thistpid, this.nowint.get(thistpid) + cs / 10);

                    for (int i = this.nowint.get(thistpid) - cs / 10; i < this.nowint.get(thistpid); ++i) {
                        AzureAPI.ChunkCoord coord = chunks.get(i);
